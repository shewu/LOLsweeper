...
public class ScoreController {
	...
	private static ArrayList<Score> getScores() {
		...
		catch(IOException e) { System.out.println("read fail"); }
		return list;
	}
	...
	public static void writeScore(Double score) {
		...
		catch(IOException e) { System.out.println("write fail"); }
	}
	...
	public static void showScorePanel() {
		...
		try {
			for(int i = 0; i < Math.min(5, scores.size()); i++) 
				display += scores.get(i).score+" "+new SimpleDateFormat("dd/MM/yyyy").format(new Date(scores.get(i).time))+"\n";
		}
